...
### Dokument projektowy v3.7 (zrewidowany)

> **Zmiany w v3.7:** porządki redakcyjne — usunięto nieaktualne komentarze odnoszące się do rzeczy wdrożonych w międzyczasie (stary model obrażeń w §10.3.1/§10.3.3, błędna suma XP w §13.1, zduplikowane pytanie o uprawę żywności), poprawiono rozjechane liczby (`items.json` ma 89 pozycji, `loot_tables.json` 20 tabel — nie 68/19 jak podawały wcześniejsze wersje).  
>   
> **Zmiany w v3.6:** **pełny zestaw przedmiotów** (`items.json`, 89 pozycji) i **tabele dropu** (`loot_tables.json`, 20 tabel). Zweryfikowano model obrażeń, ekonomię amunicji i udźwig (§10.3). Przeliczono model obrażeń na zależność podpierwiastkową z `wound_factor` zamiast liniowej (§5.1.5).  
>   
> **Zmiany w v3.5:** dodano **§20 Warstwę fabularną** — mozaikowa historia w czterech fazach plus wątek własny gracza, szkic 14 fragmentów w `story.json`, Archiwum jako ekran zbiorczy. Trzy typy przeciwników wyjaśnione jako trzy stadia choroby, bez zmiany parametrów.  
>   
> **Zmiany w v3.4:** **losowa liczba stron** dla całej literatury z wyprowadzoną masą, objętością i XP (§4.6.4); **szkic 16 dwujęzycznych notatek** w `notes.json` wraz ze schematem, podstawianiem nazw z OSM i wytycznymi redakcyjnymi (§19.1.1–19.1.3).  
>   
> **Zmiany w v3.3:** dodano **§19 Głębia przeżyciowa** — diagnoza, że gra jest kompletna systemowo, ale uboga przeżyciowo, wraz z siedmioma brakami (ślady po ludziach, wydarzenia losowe, loot jako czynność, warstwa historii na mapie, tropy, modyfikatory przeciwników, kamienie milowe).  
>   
> **Zmiany w v3.2:** **przyrost umiejętności w trakcie czytania** proporcjonalnie do stron (§4.6.1), z zabezpieczeniem przed powtórkami tytułów. ⚠️ **Poprawiono błąd arytmetyczny w krzywej XP** z v2.0 — faktyczna suma wynosiła 3,09 mln zamiast deklarowanych 82 tys.; krzywa przeliczona (§7.2).  
>   
> **Zmiany w v3.1:** **usunięto Health Connect** — sen jest automatyczny, wyprowadzony z pory doby i miejsca (§2.5); gra nie zbiera żadnych danych zdrowotnych. Wprowadzono **model zajęć: jedno działanie naraz** (§2.1a).  
>   
> **Zmiany w v3.0:** rozstrzygnięto **limity noszenia** (§18.1a — masa i objętość zamiast limitów sztukowych, nowy parametr pojemności plecaków). **Wszystkie decyzje blokujące fizjologię i walkę są zamknięte.**  
>   
> **Zmiany w v2.9:** dodano **przeszukanie terenu** jako mechanikę rozpoznania (§10.2) wraz z lornetką i trzecim efektem Zwiadu, zatwierdzono **recykling przedmiotów** (§18.6a), rozstrzygnięto lokalizację obozów.  
>   
> **Zmiany w v2.8:** rozstrzygnięto **skalę czasu** przez model stref metabolicznych (§2.1), dodano **Obóz** jako bazę wypadową (§8.5) oraz **§18 Wytwarzanie i receptury** z pełnym kosztorysem modułów i elaboracją amunicji.  
>   
> **Zmiany w v2.7:** rozwiązano cztery z pięciu zagrożeń krytycznych — **§10.1** loot proceduralny, **§11.1** trwałość zapisu i migracje, **§11.2** tryb deweloperski, **§17** sezonowość niezależna od półkuli. Ryzyko lokalizacji w tle obniżone.  
>   
> **Zmiany w v2.6:** dodano **§16 Rejestr luk** — audyt niezaprojektowanych obszarów.  
>   
> **Zmiany w v2.5:** przeliczono **model celności** na wartości realistyczne z rozstrzygnięciem konwencji MOA (§5.1), dodano **system hałasu i przyciągania przeciwników** (§5.6), domknięto model obrażeń stałą k (§5.1.5), wykluczono zakupy wewnętrzne.  
>   
> **Zmiany w v2.4:** dodano **§14 Dźwięk i haptyka** (stos, źródła, licencje, lista assetów MVP) oraz **§15 Onboarding**.  
>   
> **Zmiany w v2.3:** dodano **regułę obecności** jako zasadę ogólną dla wszystkich stanów czasowych (§2.1a) oraz obsługę przemieszczenia gracza w czasie nieprzytomności (§9.2.1).  
>   
> **Zmiany w v2.2:** rozstrzygnięto model śmierci — dwa tryby wybierane przy tworzeniu postaci: **Hardcore** (permadeath) i **Softcore** (utrata przytomności zamiast śmierci) — §9.  
>   
> **Zmiany w v2.1:** rozstrzygnięto cel gry (przetrwanie jako wynik — §13.1), zatwierdzono model pościgu z budżetem sprintu (§6.1), dodano **system ognisk** (§6.5) i **walkę z wieloma przeciwnikami** (§5.5).

**Platforma:** Android (Flutter) **Gatunek:** Hiper-realistyczny survival GPS z elementami craftingu **Model sieciowy:** offline-first, brak własnych serwerów gry **Status dokumentu:** roboczy — sekcje oznaczone ❓ wymagają decyzji projektowej
...
| Znak | Znaczenie |
| :---- | :---- |
| 🆕 | Sekcja/mechanika dodana w v2 (brak w v1) |
| ⚠️ | Zmiana wynikająca z błędu lub sprzeczności w v1 |
...
Dokument opisuje docelową grę. Do pierwszej grywalnej wersji rekomenduję zakres:
...
Woda\_bazowa \[ml/dobę\] \= 35 × masa\_ciała \[kg\]

Woda\_całkowita \= Woda\_bazowa \+ straty\_z\_potem

Straty z potem liczone dynamicznie — patrz sekcja 2.4.

**Tętno spoczynkowe** 🆕 — brak danych o kondycji gracza, więc estymujemy:

HR\_spoczynkowe \= 70 \+ 0.15 × (BMI − 22\) \+ 0.1 × (wiek − 30\)

zakres ograniczony do 50–95 bpm

⚠️ Rozważaliśmy odczyt realnego tętna spoczynkowego z Health Connect. **Odrzucone** — wymagałoby uprawnień do danych zdrowotnych, a estymata powyżej jest wystarczająco dobra dla mechaniki. Patrz §2.5.

**Tętno maksymalne — wzór Tanaki** (dokładniejszy niż 220−wiek):

HR\_max \= 208 − 0.7 × wiek

**Maksymalny udźwig** ⚠️ 📐 — v1 nie podawał wzoru. Propozycja oparta o normy marszowe wojskowe:

Udźwig\_komfortowy  \= 0.30 × masa\_ciała   (bez kar)

Udźwig\_maksymalny  \= 0.45 × masa\_ciała   (twardy limit)

Dla 80 kg: komfort 24 kg, max 36 kg.

⚠️ **Kluczowa zmiana wobec v1:** przekroczenie udźwigu komfortowego **nie blokuje** ruchu (nie możemy spowolnić realnego gracza). Zamiast tego skalujemy koszt metaboliczny — patrz 2.3.

---

## 2\. Fizjologia

### 2.1. ✅ Skala czasu i strefy metaboliczne (rozstrzygnięte w v2.8)

v1 nie definiował skali czasu ani zachowania przy zamkniętej aplikacji. Przy skali 1:1 gracz, który nie otworzy gry przez trzy dni, umiera z odwodnienia — gwarantowana utrata użytkownika.

⚠️ **Odrzucone rozwiązanie z v2.0** („10% zużycia przy zamkniętej aplikacji") było abstrakcją oderwaną od świata gry: tempo metabolizmu zależało od tego, czy proces działa, a nie od tego, co robi postać. To sprzeczne z filarem z §0.
...
### 2.1a. ✅ Zajęcia i reguła obecności (przepisane w v3.1)

#### 2.1a.1. Zasada: jedno zajęcie naraz ⚠️
...
⚠️ **To zmiana wobec v2.3**, wymuszona przez automatyczny sen. Gracz fizycznie śpi w nocy — nie może trzymać otwartej aplikacji przez osiem godzin. Gdyby zajęcia schronowe wymagały obecności, cała ekonomia czasu z §2.1a.2 byłaby fikcją.
...
kcal/min \= MET × 3.5 × masa\_ciała \[kg\] / 200

Modyfikator obciążenia 🆕:

MET\_efektywne \= MET × (1 \+ 0.8 × (obciążenie / masa\_ciała))
...
pot \[ml/h\] \= 400 \+ 200 × (MET − 1\) \+ 50 × max(0, T\_otoczenia − 20\)

             \+ modyfikator\_odzieży

gdzie `modyfikator_odzieży` \= suma izolacji noszonych elementów × 100 ml/h przy T \> 22°C. To domyka mechanikę z v1 („kurtka zimowa w 30 stopniach") liczbowo.

### 2.4. Tętno 📐

%intensywności \= (MET − 1\) / (MET\_max − 1),  MET\_max ≈ 14

HR\_aktualne \= HR\_spoczynkowe \+ %intensywności × (HR\_max − HR\_spoczynkowe)

Powrót do spoczynku po wysiłku: wykładniczy, stała czasowa τ ≈ 90 s (dla osoby przeciętnej).

🆕 **Tętno jako ukryta stamina.** v1 mówi „gracz nie posiada staminy" — i słusznie, ale tętno pełni tę rolę funkcjonalnie. Wpływ:

| HR jako % HR\_max | Efekt |
| :---- | :---- |
| \< 60% | brak kar |
...
### 2.5. ✅ Sen (rozstrzygnięte w v3.1)

⚠️ **Odrzucono integrację z Health Connect.** Powody:

- Wymagałaby uprawnień do **danych zdrowotnych** i osobnej deklaracji w Google Play  
- Health Connect niczego nie mierzy — bez smartwatcha lub dedykowanej aplikacji śledzącej sen magazyn jest pusty, więc większość graczy i tak korzystałaby ze ścieżki zapasowej  
- Gra deklaruje, że dane nie opuszczają telefonu (§1.2); **najprostszym sposobem dotrzymania tej obietnicy jest niezbieranie ich wcale**

✅ **Rozwiązanie: sen automatyczny, wyprowadzony z miejsca i pory doby.** Zero uprawnień, zero danych zdrowotnych, zero zależności od sprzętu gracza.
...
sen\_zaliczony \= min(godziny\_nocy\_w\_schronie, zapotrzebowanie\_dobowe)
...
### 2.6. Model obrażeń i krwawienia ⚠️ 🆕 — **brak w v1, blokuje sekcje 5 i 6**

v1 podaje litry krwi u gracza i przeciwników, ale nie ma przelicznika trafienia na ubytek. Bez tego walka jest niepoliczalna.
...
📐 **Modyfikator wysiłku:** `ubytek_efektywny = ubytek_bazowy × (HR_aktualne / HR_spoczynkowe)` To domyka regułę z v1 „bieganie przy krwawieniu \= szybsza utrata krwi" — przy tętnie 160 vs spoczynkowym 70 krwawienie przyspiesza 2.3×.

**Konwersja trafienia na obrażenia:**

ubytek\_natychmiastowy \[ml\] \= energia\_pocisku \[J\] × współczynnik\_lokalizacji × (1 − ochrona\_pancerza)

typ\_krwawienia \= f(lokalizacja, energia)
...
### 3.2. Jakość sygnału GPS ⚠️ 🆕

Realny problem, o którym v1 milczy: dokładność GPS w mieście to 5–15 m, wewnątrz budynków brak sygnału. Dryf pozycji przy nieruchomym graczu generuje fałszywy „ruch" → gra spala kalorie osobie siedzącej na kanapie.
...
**Dolne menu:** PROFIL · EKWIPUNEK · SCHRON · USTAWIENIA (jak w v1)
...
v1 opisuje typy przedmiotów prozą. Do implementacji potrzebny jest jeden schemat — inaczej każdy typ dostanie osobny, niekompatybilny kod.

{
  ...
  "name\_key": "item.weapon\_ak74.name",
  ...
  "condition\_decay\_per\_use": 0.05,

  "rarity": "uncommon",

  "loot\_tags": \["military", "police"\],

  "props": { }

}

Pole `props` zawiera dane specyficzne dla typu. Pliki: `weapons.json`, `food.json` ⚠️ *(w v1 literówka: `foof.md`)*, `literature.json`, `tools.json`, `armor.json`, `medical.json`, `crafting.json`.

⚠️ **Format:** rekomenduję JSON zamiast Markdown — Markdown wymaga parsera i nie ma walidacji schematu. Dokumentację opisową można trzymać w `.md` obok.

🆕 **Dodane pole `volume_l`** — sam ciężar nie wystarczy. Bez objętości gracz zmieści 30 kg puchowych kurtek w kieszeni. Plecak ma dwa limity: masę i pojemność.
...
Parametry z v1 (MOA, zasięgi, przeładowanie, tryby ognia, magazynek) \+ uzupełnienia:
...
Dwie niezależne osie ochrony (v1 je miesza):
...
### 4.6. Literatura 📐 ✅ (przepisane w v3.2)

📐 **Model czasu czytania:**

czas \[min\] \= (liczba\_stron × słów\_na\_stronę) / (tempo\_bazowe × mnożnik\_typu × (1 \+ wprawa\_czytelnicza))
...
XP\_przyznane \= XP\_książki × (strony\_przeczytane / strony\_łącznie)
...
#### 4.6.2. ⚠️ Przyrost procentowy zależy od poziomu
...
#### 4.6.4. 🆕 Losowa liczba stron (v3.4)
...
strony        \= random(pages\_min, pages\_max)          // przy generowaniu instancji

masa \[g\]      \= strony × g\_na\_stronę \+ masa\_okładki

objętość \[l\]  \= strony × l\_na\_stronę \+ objętość\_okładki

czas \[min\]    \= (strony × 280\) / (220 × mnożnik\_typu × (1 \+ wprawa))

XP            \= strony × XP\_na\_stronę
...
### 5.1. Model celności ✅ 📐 — **przeliczony na realizm w v2.5**

v1 mówił „broń ma szansę na trafienie MOA". To pojęciowy błąd: **MOA opisuje rozrzut kątowy, nie prawdopodobieństwo.**

⚠️ **Rozstrzygnięcie konwencji (brakowało w v2.0):** MOA opisuje **średnicę** grupy strzałów, nie promień. Broń „1 MOA" daje grupę o średnicy \~2,9 cm na 100 m. Pomylenie średnicy z promieniem zmienia szansę trafienia około trzykrotnie.

D\_rozrzutu \[m\] \= MOA\_total × d × 2.9089e-4      // ŚREDNICA

σ \= D\_rozrzutu / 4                              // extreme spread ≈ 4σ
...
MOA\_total \= √(MOA\_broni² \+ MOA\_umiejętności² \+ MOA\_tętna²

              \+ MOA\_ruchu\_gracza² \+ MOA\_ruchu\_celu² \+ MOA\_stanu²)

#### 5.1.1. ⚠️ Rekalibracja składników — kluczowa zmiana

Wartości z v2.0 były błędne w obie strony: umiejętność miała za duży udział, a stan fizjologiczny za mały (+5 MOA przy maksymalnym tętnie było nieistotne przy bazie 12 MOA). Efekt: strzelanie zależało od poziomu postaci, a nie od tego, co robi gracz.

**Realizm mówi coś innego: broń i wprawa to drobiazg, dominującym źródłem błędu jest stan ciała strzelca.** Nowe wartości to odzwierciedlają.
...
#### 5.1.5. Model obrażeń 📐 ⚠️ (przeliczony w v3.6)

⚠️ **Korekta wobec v2.5.** Model liniowy `J × 0.3` dawał absurdalne wyniki dla broni krótkiej: pistolet 9 mm (500 J) potrzebowałby **11 trafień** na Szwędacza. Problem jest strukturalny — ciężkość rany nie rośnie proporcjonalnie do energii, więc zależność liniowa zawsze będzie karać małe kalibry i premiować duże.

📐 **Model poprawiony — zależność podpierwiastkowa plus współczynnik rany:**

ubytek \[ml\] \= 5.1 × energia\[J\]^0.6 × wound\_factor × mnożnik\_lokalizacji × (1 − ochrona)
...
| Kaliber | wound\_factor | Energia | ml/tors |
| :---- | :---- | :---- | :---- |
| .22 LR | 0,70 | 160 J | 75 |
| 9×19 | 1,00 | 500 J | 212 |
...
### 5.4. Walka wręcz ⚠️ 🆕 — brak modelu w v1

P(trafienie) \= 0.65 \+ 0.30 × umiejętność − 0.25 × (obciążenie / udźwig\_max)
...
### 5.5. Walka z wieloma przeciwnikami 🆕 — **brak w v1 i v2.0**

Ani opis pierwotny, ani v2.0 nie definiowały, co się dzieje, gdy przeciwników jest więcej niż jeden. A Szwędacze występują w grupach 2–4, więc to jest domyślny stan walki, nie przypadek szczególny.
...
czas\_przełączenia \= 1.2 s × (1 − 0.30 × Obsługa\_Broni)
...
### 5.6. Hałas i przyciąganie przeciwników 🆕 — nowy system (v2.5)
...
promień\_efektywny \= promień\_bazowy × mod\_pory\_doby × mod\_zabudowy
...
### 6.1. ⚠️ KRYTYCZNA korekta balansu prędkości

**To najpoważniejszy problem w v1.** Skoro prędkość gracza jest równa jego realnej prędkości, każdy przeciwnik szybszy od \~12 km/h jest niemożliwy do ucieczki — dla przeciętnego gracza granica jest bliżej 9 km/h.

Wartości z v1: Skakun 27–32 km/h (szybciej niż rekord świata Usaina Bolta — 37 km/h w szczycie, ale \~10 s), Brutal 12–17 km/h. Każde spotkanie kończy się złapaniem gracza. Ucieczka — podstawowa opcja taktyczna w survivalu — przestaje istnieć.

✅ **DECYZJA (v2.1): prędkości z v1 zostają bez zmian, wprowadzamy budżet sprintu.**
...
### 6.2. Parametry przeciwników (uzupełnione)
...
⚠️ **„Mnożnik ataku" z v1 (0.9 / 1.05 / 1.2) nie miał zdefiniowanej bazy** — dodano wiersz obrażeń bazowych w ml, do którego mnożnik się odnosi.

### 6.3. Poruszanie się przeciwników ⚠️ 🆕
...
interwał\_awansu \= max(2h, 8h − 0.25h × dzień\_przetrwania) × random(0.6, 1.4)
...
Integralność\_max(poziom) \= 60 \+ 20 × poziom
...
#### Zbicie poziomu — mechanika „wzburzenia" ⚠️
...
Cztery umiejętności z v1, zakres 0–100%, maksymalny efekt \+30%.

| Umiejętność | Efekt przy 100% |
| :---- | :---- |
| **Zwiad** | −30% szansy na wykrycie, \+30% jakości lootu, **\+100% promienia przeszukania terenu** (§10.2) |
| **Obsługa broni** | −30% czasu przeładowania, −30% czasu stabilizacji, MOA\_umiejętności 12→2 |
| **Medycyna** | −30% czasu użycia medykamentów, \+30% skuteczności opatrunków |
| **Inżynieria** | −30% czasu budowy i craftingu |

### 7.1. ⚠️ Rozdzielenie tempa czytania

v1 wiąże tempo czytania literatury z umiejętnością Medycyna. To nieintuicyjne — czytanie encyklopedii broni nie powinno zależeć od wiedzy medycznej.

🆕 **Propozycja:** wydzielić tempo czytania jako osobny modyfikator rosnący z ogólnej liczby przeczytanych stron („wprawa czytelnicza", max −25% czasu). Umiejętność Medycyna zachowuje wpływ wyłącznie na medykamenty.

### 7.2. ✅ Model progresji (przeliczony w v3.2)

⚠️ **Korekta błędu z v2.0.** Podana tam krzywa `XP(p) = 50 × p^1.6` z sumą „≈82 000 XP" była błędna arytmetycznie — faktyczna suma tej krzywej to **3 087 599 XP**, czyli \~2 900 godzin czytania na jedną umiejętność. Nie do osiągnięcia w żadnej passie.

📐 **Nowa krzywa — liniowo rosnący koszt:**

XP\_do\_poziomu(p) \= 70 × p          // p \= 1..100

XP\_łączne do 100% \= 353 500
...
### 8.1. ⚠️ Korekta stref — sprzeczność w v1

v1: przeciwnicy nie wchodzą w promień 30 m, gracz nie może atakować w promieniu 50 m.

**Powstaje pierścień 30–50 m, w którym przeciwnik może atakować gracza, a gracz nie może się bronić.** To wprost karze przebywanie pod schronem.
...
Efekt zgodny z intencją (brak kempienia ze schronu, brak ostrzeliwania przeciwników z bezpiecznej pozycji), bez luki. Przeciwnicy czekają przy granicy strefy — gracz musi wyjść, żeby walczyć, ale nie jest bezbronny.
...
To rozwiązuje trzy problemy naraz: skalę czasu offline (§2.1), tempo gry dla pracującego dorosłego (§16.8) i brak sensownej rozgrywki w godzinach pracy.
...
### 9.2. Tryb SOFTCORE — którą wersję wybrać

Rozważaliśmy dwa warianty. Poniżej ocena obu i rekomendacja.

**Wariant A — utrata przedmiotów, natychmiastowe odrodzenie w schronie** Prosty w implementacji, ale ma dwie wady. Po pierwsze, teleportacja do schronu łamie podstawową zasadę gry, w której pozycja postaci \= realna pozycja gracza. Po drugie, kara jest wyłącznie księgowa: „minus plecak" bez żadnego przeżycia po stronie gracza.

**Wariant B — nowe życie dopiero po powrocie do schronu** Spójny fabularnie, ale blokuje rozgrywkę: gracz, który zginął w porze obiadowej 2 km od domu, nie może grać do wieczora. To najpewniejsza droga do porzucenia aplikacji.

✅ **Rekomendacja: wariant C — utrata przytomności zamiast śmierci.**
...
#### 9.2.1. Przemieszczenie gracza w czasie nieprzytomności ⚠️ 🆕
...
**Dlaczego to jest lepsze od A i B:**
...
⚠️ **Świadomie rezygnujemy z utraty XP** obecnej w v2.0. Fikcja „zapominasz, jak się strzela, bo zemdlałeś" jest słaba, a utrata sprzętu plus wyzerowana passa to wystarczająca stawka.
...
v1 wspomina lootboxy, ale nie definiuje ich rozmieszczenia. Propozycja, która wykorzystuje przewagę gry GPS-owej:
...
### 10.1. ✅ Zapasowa warstwa lootu proceduralnego (v2.6)
...
gęstość\_POI \= liczba lootowalnych POI w promieniu 2 km od schronu

if gęstość\_POI \< 8:

    tryb\_zapasowy \= ON

    liczba\_punktów\_zastępczych \= 12 − gęstość\_POI
...
| \`building=house | detached\` | opuszczony dom |
| \`building=barn | farm\_auxiliary\` | stodoła |
...
### 10.3. ✅ Zawartość: przedmioty i tabele dropu (v3.6)

Pliki: **`items.json`** (89 przedmiotów) i **`loot_tables.json`** (20 tabel: 11 POI z OSM \+ 9 punktów proceduralnych z §10.1).

#### 10.3.1. ✅ Weryfikacja modelu obrażeń (przeliczona w v3.7)

⚠️ **Tabela poniżej zastępuje wersję z v3.6**, która wciąż używała odrzuconego modelu liniowego `k = 0,3 ml/J` (§5.1.5 — poprawiony na `5,1 × J^0,6 × wound_factor`). Wnioski jakościowe się nie zmieniły, ale liczby tak — w szczególności pistolet przestał być kompletnie bezużyteczny.

Trafienia w tors, cel bez pancerza, wobec progów krwi z §6.2:
...
#### 10.3.3. ✅ Ekonomia amunicji — potwierdzona (przeliczona w v3.7)
...
Symulacja 400 przeszukań obiektu wojskowego (głębokie, Zwiad 50%): średnio **15,8 naboi 5,45 na przeszukanie**, czyli **\~1,5 przeszukania na jedną grupę Szwędaczy**, przy respawnie co 14–24 h. Amunicja pozostaje wąskim gardłem zgodnie z założeniem — nie zmieniło się jakościowo, tylko dokładność liczb.

Amunicja karabinowa występuje wyłącznie w obiektach wojskowych, sklepach myśliwskich i na ambonach. Komisariaty dają niemal wyłącznie 9 mm — czyli dużo amunicji do słabszej broni, co teraz ma sens: pistolet przestał być bezużyteczny, więc ten nadmiar faktycznie się przydaje.
...
promień \= 100 m
        ...
        × mod\_lornetki                    // 1.0 lub 1.5

        × (1 − 0.5 × D)                   // ciemność, §17.4

        × mod\_pogody                      // mgła 0.6, opady 0.8
...
#### 10.2.3. Co odsłania — a czego nie ⚠️
...
### 11.1. ✅ Trwałość zapisu i migracje (v2.6) — odpowiedź na §16.3
...
### 11.2. ✅ Tryb deweloperski (v2.6) — must have, od pierwszego dnia
...
| **Nakładka diagnostyczna** | MOA\_total z rozbiciem na składniki, szansa trafienia, promień hałasu, stany maszyny przeciwników |
...
| \# | Decyzja | Blokuje |
| :---- | :---- | :---- |
| ~~1~~ | ~~Skala czasu i zachowanie przy zamkniętej aplikacji~~ | ✅ **rozstrzygnięte w v2.8 — strefy metaboliczne (§2.1)** |
| ~~2~~ | ~~Model śmierci~~ | ✅ **rozstrzygnięte w v2.2 — Hardcore \+ Softcore (9)** |
| ~~3~~ | ~~Wykrywanie snu~~ | ✅ **rozstrzygnięte w v3.0 — dwie ścieżki, §2.5.3** |
| ~~4~~ | ~~Wariant balansu prędkości przeciwników~~ | ✅ **rozstrzygnięte w v2.1 — budżet sprintu (6.1)** |
| 5 | Ograniczenia lokalizacji schronu (8.2) | prywatność i fabułę |
| 6 | Tryb dostępności (12) | zakres grupy docelowej |
| ~~7~~ | ~~Cel długoterminowy rozgrywki~~ | ✅ **rozstrzygnięte w v2.1 — przetrwanie (13.1)** |
| 8 | Zawór bezpieczeństwa wzburzenia — czy 400 m wystarczy (6.5.4) | balans neutralizacji |
| ~~10~~ | ~~Zakupy wewnętrzne~~ | ✅ **rozstrzygnięte w v2.5 — BRAK IAP** |
| 11 | Model dystrybucji: darmowa / płatna jednorazowo | ⏸ odroczone do po MVP — obie ścieżki otwarte, konto Google Play opłacone |
| 12 | Promień hałasu karabinu w gęstej zabudowie (5.6.5) | balans walki |

✅ **Decyzja 10 — brak zakupów wewnętrznych.** Konsekwencje: nie wdrażamy Google Play Billing, nie ma waluty premium, nie ma skrótów za pieniądze. Architektura stanu upraszcza się (brak serwera weryfikacji zakupów), a projekt pozostaje spójny z filarem z §0 — mikropłatności w grze o realnej fizjologii przetrwania podważałyby wszystko, na czym stoi.

⚠️ Decyzja 11 nie wpływa na kod. Płatność jednorazowa to ustawienie w Play Console, nie funkcja aplikacji — można ją podjąć w dniu publikacji.

### 13.1. ✅ Cel gry: przetrwanie (rozstrzygnięte w v2.1)
...
| :---- | :---- | :---- |
| Silnik audio | **flutter\_soloud** | niskopoziomowy plugin oparty na silniku SoLoud (C++), przeznaczony do gier; komunikacja przez `dart:ffi`, bez method channels — stąd niskie opóźnienie |
| Dźwięk pozycyjny | `play3d()` z flutter\_soloud | pakiet ma wbudowane audio 3D z pozycjonowaniem źródeł i konfiguracją słuchacza |
...
x \= (lon\_wroga − lon\_gracza) × 111320 × cos(lat\_gracza)

z \= (lat\_wroga − lat\_gracza) × 110540
...
Głośność bicia rośnie z tętnem: niesłyszalne poniżej 60% HR\_max, dominujące powyżej 90%.
...
Sekcje wymienione niżej **nie są jeszcze zaprojektowane**. To rejestr świadomych braków, nie propozycji — służy temu, żeby żaden z nich nie został odkryty dopiero przy publikacji.

### 16.1. ✅ Gracz poza dużym miastem — ROZWIĄZANE (§10.1)

Cała ekonomia (§10) opiera się na POI z OpenStreetMap: apteki, markety, biblioteki, komisariaty. **W miejscowości poniżej \~15 tys. mieszkańców tych obiektów praktycznie nie ma.** Gracz z małego miasta albo wsi nie ma czego lootować — gra dla niego nie działa w ogóle.

To nie jest kwestia balansu, tylko wielkości rynku.

✅ **Rozwiązane w v2.6:** zapasowa warstwa lootu proceduralnego — pełny projekt w **§10.1**. Wyzwalana przy gęstości POI \< 8 w promieniu 2 km, oparta o obiekty obecne wszędzie (samochody, stodoły, garaże, ambony myśliwskie), z rekompensatą w postaci szybszego respawnu i większego promienia poszukiwań.

### 16.2. 🟡 Uprawnienie do lokalizacji w tle — ryzyko obniżone

Android wymaga osobnej zgody `ACCESS_BACKGROUND_LOCATION`, a Google Play — odrębnego formularza uzasadnienia i weryfikacji.

✅ **Ryzyko obniżone:** proces przeszedł już pomyślnie za pierwszym podejściem w innej aplikacji tego samego dewelopera. Pozostaje standardowa staranność: wideo demonstracyjne, precyzyjne uzasadnienie, deklaracja spójna z faktycznym użyciem.
...
### 16.3. ✅ Trwałość zapisu i migracje — ROZWIĄZANE (§11.1)

✅ **Rozwiązane w v2.6.** Pełny projekt w **§11.1**: trójwarstwowy podział stanu wg częstotliwości zapisu (maks. strata 60 s), SQLite w trybie WAL z transakcjami i idempotentnymi tickami, 3 migawki rotacyjne z sumami kontrolnymi, migracje wyłącznie addytywne z testami w CI, obsługa ubijania procesu przez system.

Zasada nadrzędna: **awaria techniczna nigdy nie może zabić postaci w trybie Hardcore.**

### 16.4. ✅ Sezonowość — ROZWIĄZANE (§17)

✅ **Rozwiązane w v2.6.** Pełny projekt w **§17**. Kluczowe rozstrzygnięcie: gra **nie zna pojęcia pory roku** — wyprowadza wszystkie modyfikatory z dwóch mierzonych wielkości (długość dnia liczona z szerokości geograficznej, temperatura z Open-Meteo). Sezonowość powstaje sama i poprawnie na obu półkulach, a w strefie równikowej znika.

⚠️ **Zależność otwarta:** rozwiązanie stoi na założeniu, że jest co robić w schronie przez cztery miesiące. Obecnie nie ma — patrz §17.6.

### 16.5. ✅ Tryb deweloperski — ZATWIERDZONE JAKO MUST HAVE (§11.2)

✅ **Rozwiązane w v2.6.** Pełny projekt w **§11.2**, zakres: symulator GPS z odtwarzaniem GPX, przyspieszenie czasu ×3600, panele fizjologii i świata, nakładka diagnostyczna z rozbiciem MOA, deterministyczne powtórki, zestawy testowe.

⚠️ Symulator musi wstrzykiwać dane przez tę samą warstwę co prawdziwy GPS, **razem z symulacją błędu pozycji i utraty sygnału** — testowanie na idealnych danych zamaskuje wszystko, co wystąpi w terenie.

### 16.6. 🟡 Spirala niemocy w trybie Softcore
...
✅ **Częściowo rozwiązane w v2.8:** Obóz (§8.5) usuwa karę za godziny pracy, a droga z pracy do domu staje się główną pętlą rozgrywki. Pozostaje policzyć, czy tempo wzrostu ognisk jest dopasowane do \~1 h gry dziennie.

Do zweryfikowania symulacją przed testami terenowymi.

### 16.9. 🟡 Telemetria bez serwerów
...
### 16.10. 🟡 Pierwsze uruchomienie i pakiety map
...
### 16.11. 🟢 Warstwa prawna — blokuje publikację

| Element | Uwagi |
| :---- | :---- |
| Polityka prywatności | wymagana przez Google Play, publiczny URL; musi opisywać dane lokalizacyjne i zdrowotne |
| Formularz Data Safety | deklaracja: dane zdrowotne i lokalizacja przetwarzane lokalnie |
...
| Uzasadnienie lokalizacji w tle | §16.2 |

🆕 **Uproszczenie z v3.1:** rezygnacja z Health Connect usuwa z tej listy deklarację dostępu do danych zdrowotnych i związaną z nią weryfikację — zostaje wyłącznie lokalizacja.
...
## 17\. Sezonowość i środowisko ✅ 🆕 (v2.6)
...
C \= clamp((10 − T\_śr7) / 25, 0, 1\)      // 10°C → 0.0 ; −15°C → 1.0

📐 **Wskaźnik ciemności** (0 \= długi dzień, 1 \= krótki):

D \= clamp((12 − godziny\_światła) / 6, 0, 1\)   // 12 h → 0.0 ; ≤6 h → 1.0
...
| ~~Tempo czynności schronowych~~ | ⚠️ **usunięte w v3.1** | podwójne liczenie — dłuższa noc daje więcej godzin na zajęcia (§2.1a.2), premia do tempa byłaby nagrodą za to samo dwa razy |
...
| **Listopad–luty** | C≈0.5, D≈0.8 | Sezon przygotowań. Ogniska rosną \~1,6× wolniej, przeciwnicy o 15% wolniejsi, czynności schronowe o 16% szybsze, loot odnawia się szybciej. |
...
### 17.6. ⚠️ Warunek konieczny — głębia czynności schronowych
...
✅ **Częściowo rozwiązane w v2.8 (§18):** pełna rozbudowa modułów to 731 kg materiału i \~30 kursów — zajęcie na tygodnie, naturalnie wpisujące się w sezon przygotowań. Elaboracja amunicji i receptury przedmiotów dodatkowo zapełniają czas w schronie.
...
## 18\. Wytwarzanie i receptury 🆕 (v2.8)

### 18.1. Surowce podstawowe

Cztery surowce zbierane podczas eksploracji (§4.7) plus jeden rzadki, wprowadzony w v2.8.
...
### 18.1a. ⚠️ Limity noszenia: masa i objętość, bez limitów sztukowych (v3.0)

Rozważaliśmy twardy limit liczby sztuk (np. maks. 30 drewna). **Odrzucony z dwóch powodów.**

**1\. Rozbija własne receptury.** Magazyn L3 wymaga 36 drewna, Warsztat L3 — 42 metalu. Przy limicie 30 obu modułów nie da się zbudować.

**2\. W plecaku nigdy by nie zadziałał** — masa wiąże znacznie wcześniej:
...
✅ **Rozwiązanie: dwa niezależne limity — masa i objętość plecaka.** Stackowanie bez ograniczeń sztukowych; limit wynika wyłącznie z fizyki.
...
czas \= czas\_bazowy
     ...
     × mod\_narzędzi                      // tabela wyżej

     × mod\_warsztatu                     // L1 0.90 / L2 0.80 / L3 0.70 (§8.4)
...
⚠️ Modyfikator sezonowy usunięty w v3.1 — zimą zyskuje się **godziny**, nie tempo (§2.1a.2).
...
### 18.6. ✅ Recykling przedmiotów (v2.9)

Rozbiórka przedmiotu na surowce, zatwierdzona.
...
## 19\. Głębia przeżyciowa — czego brakuje 🆕 (v3.3)
...

{
  ...
  "id": "note\_pharmacy\_01",

  "category": "kartka\_na\_drzwiach",

  "poi\_tags": \["amenity=pharmacy"\],

  "pages\_min": 1, "pages\_max": 1,
...
### 19.8. ⚠️ Ryzyko nadrzędne: gra złożona z samych kosztów
...
├── food.json          (v1: "foof.md" — literówka)
...
*Dokument v3.7 — rewizja opisu koncepcyjnego v1. Zmiany oznaczone 🆕 (dodane), ⚠️ (korekty błędów i sprzeczności), ❓ (otwarte decyzje), ✅ (rozstrzygnięte).*  
